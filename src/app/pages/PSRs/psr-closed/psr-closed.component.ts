...
@Component({
  selector: 'app-psr-closed',
  templateUrl: './psr-closed.component.html',
  styleUrls: ['./psr-closed.component.css']
})
export class PsrClosedComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  createDisplayVariable() {
    let final = JSON.parse(JSON.stringify(this.closed));
  ...
  getSeverity() {
    for (let i = 0; i < this.closed.length; i++) {
      let col = 4;
      if (this.closed[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.closed[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.closed[i][col] == "Low") {
