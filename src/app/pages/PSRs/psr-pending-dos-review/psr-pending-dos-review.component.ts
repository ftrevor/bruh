...
@Component({
  selector: 'app-psr-pending-dos-review',
  templateUrl: './psr-pending-dos-review.component.html',
  styleUrls: ['./psr-pending-dos-review.component.css']
})
export class PSRPendingDosReviewComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  createDisplayVariable() {
    let final = JSON.parse(JSON.stringify(this.closedPendingRequestorVerification));
  ...
  getSeverity() {
    for (let i = 0; i < this.closedPendingRequestorVerification.length; i++) {
      let col = 4;
      if (this.closedPendingRequestorVerification[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.closedPendingRequestorVerification[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.closedPendingRequestorVerification[i][col] == "Low") {
