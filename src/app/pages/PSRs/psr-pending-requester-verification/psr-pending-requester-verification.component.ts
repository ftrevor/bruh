...
@Component({
  selector: 'app-psr-pending-requester-verification',
  templateUrl: './psr-pending-requester-verification.component.html',
  styleUrls: ['./psr-pending-requester-verification.component.css']
})
export class PSRPendingRequesterVerificationComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  createDisplayVariable() {
    let final = JSON.parse(JSON.stringify(this.pendingRequestorVerification));
  ...
  getSeverity() {
    for (let i = 0; i < this.pendingRequestorVerification.length; i++) {
      let col = 4;
      if (this.pendingRequestorVerification[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.pendingRequestorVerification[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.pendingRequestorVerification[i][col] == "Low") {
