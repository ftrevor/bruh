...
@Component({
  selector: 'app-rfas-pending-dos-review',
  templateUrl: './rfas-pending-dos-review.component.html',
  styleUrls: ['./rfas-pending-dos-review.component.css']
})
export class RfasPendingDosReviewComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  pendingGDIT: any[] = [];
  pendingDOS: any[] = [];
  ...
  getCondensedCategories(){
    ...
    let final = JSON.parse(JSON.stringify(this.condensedPendingDOS));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedPendingDOS.length; i++) {
      let col = 3;
      if (this.condensedPendingDOS[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedPendingDOS[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedPendingDOS[i][col] == "Low") {
