...
@Component({
  selector: 'app-reports-automated-report',
  templateUrl: './reports-automated-report.component.html',
  styleUrls: ['./reports-automated-report.component.css']
})
export class ReportsAutomatedReportComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  getCondensedCategories() {
    ...
    let final = JSON.parse(JSON.stringify(this.condensedCompletedAutomatedReports));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedCompletedAutomatedReports.length; i++) {
      let col = 3;
      if (this.condensedCompletedAutomatedReports[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedCompletedAutomatedReports[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedCompletedAutomatedReports[i][col] == "Low") {
