...
@Component({
  selector: 'app-reports-awaiting-dc',
  templateUrl: './reports-awaiting-dc.component.html',
  styleUrls: ['./reports-awaiting-dc.component.css']
})
export class ReportsAwaitingDcComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  getCondensedCategories() {
    ...
    let final = JSON.parse(JSON.stringify(this.condensedCompletedAwaitingDC));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedCompletedAwaitingDC.length; i++) {
      let col = 3;
      if (this.condensedCompletedAwaitingDC[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedCompletedAwaitingDC[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedCompletedAwaitingDC[i][col] == "Low") {
