...
@Component({
  selector: 'app-defects-closed-fixed',
  templateUrl: './defects-closed-fixed.component.html',
  styleUrls: ['./defects-closed-fixed.component.css']
})
export class DefectsClosedFixedComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  getCondensedCategories(){
    ...
    let final = JSON.parse(JSON.stringify(this.condensedClosedFixed));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedClosedFixed.length; i++) {
      let col = 3;
      if (this.condensedClosedFixed[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedClosedFixed[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedClosedFixed[i][col] == "Low") {
