...
@Component({
  selector: 'app-defects-closed-not-a-defect',
  templateUrl: './defects-closed-not-a-defect.component.html',
  styleUrls: ['./defects-closed-not-a-defect.component.css']
})
export class DefectsClosedNotADefectComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  getCondensedCategories() {
    ...
    let final = JSON.parse(JSON.stringify(this.condensedClosedNotADefect));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedClosedNotADefect.length; i++) {
      let col = 3;
      if (this.condensedClosedNotADefect[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedClosedNotADefect[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedClosedNotADefect[i][col] == "Low") {
