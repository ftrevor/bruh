...
@Component({
  selector: 'app-dev-ops-defects-completed-fixed-in-test-passed',
  templateUrl: './dev-ops-defects-completed-fixed-in-test-passed.component.html',
  styleUrls: ['./dev-ops-defects-completed-fixed-in-test-passed.component.css']
})
export class DevOpsDefectsCompletedFixedInTestPassedComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  getCondensedCategories(){
    ...
    let final = JSON.parse(JSON.stringify(this.condensedCompleted));
  ...
  getSeverity() {
    for (let i = 0; i < this.condensedCompleted.length; i++) {
      let col = 3;
      if (this.condensedCompleted[i][col] == "High") {
        this.serverityHigh = this.serverityHigh + 1;
      }
      else if (this.condensedCompleted[i][col] == "Medium") {
        this.serverityMedium = this.serverityMedium + 1;
      }
      else if (this.condensedCompleted[i][col] == "Low") {
