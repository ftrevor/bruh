...
@Component({
  selector: 'app-dev-ops-defects-pending-dos-dohaction',
  templateUrl: './dev-ops-defects-pending-dos-dohaction.component.html',
  styleUrls: ['./dev-ops-defects-pending-dos-dohaction.component.css']
})
export class DevOpsDefectsPendingDOSDOHActionComponent implements OnInit {
  // Raw Data
  excelData: any[] = [];
  ...
  addAgeColumns() {
    this.seperate = this.pendingDOSAction.map(row => row.slice());
  ...
  getCondensedCategories() {
    ...
    let final = JSON.parse(JSON.stringify(this.condensedPendingDOSAction));
